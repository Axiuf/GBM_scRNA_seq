...
plot_dir <- "./plots/05_GBM_total_merge_Marker"
dir.create(plot_dir)
...
# Add group information to meta.data, for convenience of marker genes identification
GBM$cell_type <- NA
...
Primary_samples <- c("P609", "P673", "P673_jc", "P690", "P828_CD133Neq", "P828_CD133Posi","P843_CD133Neg", 
                     "P843_CD133posi", "P873", "P912_left0514", "P912_right0514", "P924")
Recurrent_samples <- c("P625", "P653", "P653_jc", "P882", "P887")
Secondary_samples <- c("P632", "P645", "P679_yang", "P679_yin", "P711")
...
GBM$infercnv_type_tumor_patient[GBM$infercnv_type_tumor_patient %in% Primary_samples] <- 
  paste0("Primary_", GBM$infercnv_type_tumor_patient[GBM$infercnv_type_tumor_patient %in% Primary_samples])
GBM$infercnv_type_tumor_patient[GBM$infercnv_type_tumor_patient %in% Secondary_samples] <- 
  paste0("Secondary_", GBM$infercnv_type_tumor_patient[GBM$infercnv_type_tumor_patient %in% Secondary_samples])
GBM$infercnv_type_tumor_patient[GBM$infercnv_type_tumor_patient %in% Recurrent_samples] <- 
  paste0("Recurrent_", GBM$infercnv_type_tumor_patient[GBM$infercnv_type_tumor_patient %in% Recurrent_samples])
...
# Macrophage
FeaturePlot(GBM, features = c("CD14", "AIF1", "FCER1G", "FCGR3A", "TYROBP", "CSF1R"), reduction = "umap",
...
# T cell
FeaturePlot(GBM, features = c("CD2", "CD3D", "CD3E", "CD3G"), reduction = "umap",
            min.cutoff = 'q15', pt.size = 0.1, label = TRUE, order = TRUE)
ggsave(filename = "T_cell_FeaturePlot.tiff",
       device = "tiff", path = plot_dir,width = 16, height = 14, dpi = fig_dpi)

# Oligodendrocyte
FeaturePlot(GBM, features = c("MBP", "TF", "PLP1", "MAG", "MOG", "CLDN11"), reduction = "umap",
            min.cutoff = 'q15', pt.size = 0.1, label = TRUE, order = TRUE)
...
FeaturePlot(GBM, features = c("CD34", "ESAM", "CD53", "CD74", "MOG", "MBP"), reduction = "umap",
            min.cutoff = 'q15', pt.size = 0.1, label = TRUE, order = TRUE)
ggsave(filename = "Normal_cell_markers_FeaturePlot.tiff",
       device = "tiff", path = plot_dir,width = 16, height = 21, dpi = fig_dpi)
...
metadata %>% 
  ...
  ggtitle("nCells")
ggsave(filename = "cluster0.2_by_sample_geom_bar.tiff",
       device = "tiff", path = plot_dir, width = 8, dpi = fig_dpi)
...
metadata %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
  ggtitle("nCells")
ggsave(filename = "sample_by_cluster0.2_geom_bar.tiff",
       device = "tiff", path = plot_dir, width = 8, dpi = fig_dpi)
...
metadata %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
  ggtitle("nCells")
ggsave(filename = "cell_type_geom_bar.tiff",
       device = "tiff", path = plot_dir, width = 8, dpi = fig_dpi)


# Plot by tumor type
DimPlot(GBM, reduction = "umap", pt.size = 0.1, group.by = "tumor_type", label = TRUE)
ggsave(filename = "tumor_type_DimPlot.tiff",
       device = "tiff", path = plot_dir, width = 8, dpi = fig_dpi)
