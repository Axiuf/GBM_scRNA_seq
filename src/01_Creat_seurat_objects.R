# Creat seurat objects from the output of Cell ranger (10 X)
...
GBM_objects <- lapply(GBM_sample_names, function(GBM_sample_name){
  counts_10x <- Read10X(data.dir = paste0("./data/Cellranger/",
                                          GBM_sample_name,
...
# Merge toal objects
GBM_total_merge <- merge(GBM_objects[[1]], y = GBM_objects[2:22],
...
primary_total_merge <- merge(GBM_objects[[1]], y = GBM_objects[c(7, 8, 11, 13:17, 20:22)],
...
recurrent_total_merge <- merge(GBM_objects[[2]], y = GBM_objects[c(5, 6, 18, 19)],
...
Secondary_total_merge <- merge(GBM_objects[[3]], y = GBM_objects[c(4, 9, 10, 12)],
